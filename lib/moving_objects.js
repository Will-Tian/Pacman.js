(function() {
  ...
  var MovingObjects = App.MovingObjects.MovingObjects = function MovingObjects(pos, vel, sprites, startingPos) {
    ...
    this.spriteSources = {
    	...
    	blinkyLeft: [[650, 200, 48, 50], [650, 250, 48, 50]],
    	blinkyRight: [[650, 0, 48, 50], [650, 50, 48, 50]],
    	blinkyUp: [[650, 300, 48, 50], [650, 350, 48, 50]],
    	blinkyDown: [[650, 100, 48, 50], [650, 150, 48, 50]],
    	pinkyLeft: [[700, 200, 48, 50], [700, 250, 48, 50]],
    	pinkyRight: [[700, 0, 48, 50], [700, 50, 48, 50]],
    	pinkyUp: [[700, 300, 48, 50], [700, 350, 48, 50]],
    	pinkyDown: [[700, 100, 48, 50], [700, 150, 48, 50]],
    	inkyLeft: [[750, 200, 48, 50], [750, 250, 48, 50]],
    	inkyRight: [[750, 0, 48, 50], [750, 50, 48, 50]],
    	inkyUp: [[750, 300, 48, 50], [750, 350, 48, 50]],
    	inkyDown: [[750, 100, 48, 50], [750, 150, 48, 50]],
    	clydeLeft: [[800, 200, 48, 50], [800, 250, 48, 50]],
    	clydeRight: [[800, 0, 48, 50], [800, 50, 48, 50]],
    	clydeUp: [[800, 300, 48, 50], [800, 350, 48, 50]],
    	clydeDown: [[800, 100, 48, 50], [800, 150, 48, 50]],
  ...
  MovingObjects.prototype.draw = function() {
  	if(Object.keys(App.StationaryObjects.Objects).length <= 1) {
  ...
  MovingObjects.prototype.triggerNextLevel = function() {
 	clearInterval(App.initializer.gameView.timerId);
	clearInterval(App.initializer.gameView.animationId);
	if(App.initializer.levelCounter === 4){
		App.initializer.triggerWin();
		return;
	}
	App.initializer.startNextLevel();	
 }
 ...
 MovingObjects.prototype.warp = function() {
  ...
  var Pacman = App.MovingObjects.Pacman = function Pacman(pos, vel, sprites, startingPos, ptx, gtx, dotstx){
  	...
  	this.lifeCounter = App.initializer.pacmanLifeCounter;
  };
...
Pacman.prototype.eatDot = function() {
 	if(App.StationaryObjects.Objects[this.pos] && App.StationaryObjects.Objects[this.pos].constructor.name === "SmallDot"){
 		...
 		App.initializer.pacmanScoreCounter += 50;
 		...
 		App.initializer.pacmanScoreCounter += 200;
 		$.each(App.MovingObjects.Objects, function(objectKey, object){
 ...
 Pacman.prototype.proximityTo = function(targetPos) {
 	...
 	return distance < App.Blocks.BLOCK_DIM[0] + 10;
 }
	...
	$.each(App.MovingObjects.Objects, function(objectKey, object){
  		if(object.constructor.prototype.__proto__.constructor.name === "Ghost" && this.proximityTo(object.pos)) {
  			if(object.frightened && !object.respawning){
  				App.initializer.eatGhostSound.play();
  				...
  				App.initializer.deathSound.play();
  				App.StationaryObjects.Objects['scoreBoard'].lifeCounter -= 1;
  				App.initializer.pacmanLifeCounter -= 1;
  				this.lifeCounter -= 1;
  			}
 ...
 Pacman.prototype.triggerLoss = function() {
	...
	clearInterval(App.initializer.gameView.timerId);
	clearInterval(App.initializer.gameView.animationId);
	$('.canvas').fadeTo(1000, 0.2, function(){
		$('.in-game-options-loss').show();
	})
 }
