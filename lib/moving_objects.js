(function() {
  ...
  var MovingObjects = App.MovingObjects.MovingObjects = function MovingObjects(pos, vel, sprites, ptx, gtx, pdtx, pinkytx) {
  	this.pos = pos;
  	this.vel = vel;
  	this.ptx = ptx;
  	this.gtx = gtx;
  ...
  Pacman.prototype.closestIntersection = function(currentDirection){
  	var startTile = this.currentTilePos(this.pos)
  	var found = false;
  	var accumulator = 1;
  	while(!found){
  		var currentPos = [0, 0]
  		currentPos[0] += startTile[0] + accumulator * currentDirection[0] * App.Blocks.BLOCK_DIM[0];
  		currentPos[1] += startTile[1] + accumulator * currentDirection[1] * App.Blocks.BLOCK_DIM[1];	
  		if(App.Boards.Intersections['' + JSON.stringify(currentPos)]){
  			found = true;
  		}
  		accumulator += 1
  	}
