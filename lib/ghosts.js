(function() {
  ...
  Blinky.prototype.findPath = function(st, offset) {
  	...
  	if(offset) {
  		...
  		destination = [destination[0] + dirX * 2 * BLOCK_DIM[0], destination[1] + dirY * 2 * BLOCK_DIM[0]]
  	}
  ...
  Blinky.prototype.move = function(){
  	...
  	if(App.Boards.CageCorridor['' + JSON.stringify(pacmanCurrentTile)]){
  		this.findPath(currentTilePos, true)
