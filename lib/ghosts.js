(function() {
  ...
  Ghost.prototype.findPath = function(options) {
  	...
  	if(destination[0] < App.Boards.levelDim[0][0] + 1) {
  		destination[0] = App.Boards.levelDim[0][1];
  		destination = this.currentTilePos(destination);
  	} else if(destination[0] > App.Boards.levelDim[0][1]){
  		destination[0] = App.Boards.levelDim[0][0] + 1;
  		destination = this.currentTilePos(destination);
  	}
  	var cameFrom = App.PathFinding.bfs(startNode, start, destination);
  ...
  Ghost.prototype.flee = function() {
    ...
    if(this.animationCycle - this.frightenedTimer > 36) {
      if((this.animationCycle - this.frightenedTimer) % 4 < 2) {
  ...
  Blinky.prototype.chase = function(){
  	...
  	if((this.pos.equals(pacmanCurrentTile) && !this.respawning)) {
  ...
  Pinky.prototype.move = function() {
  	...
  	if(!blinky.moved || blinky.animationCycle < 16) {
  		return;
  	}
  	if(pacman.respawning) {
  		return;
  ...
  Pinky.prototype.chase = function(){
	var pacman = App.MovingObjects.Objects['pacman'];	
  	...
  	if((this.pos.equals(pacman.closestIntersection(pacman.vel, 4)) && !this.respawning)) {
  		var nextMove = this.bestNextMove(pacman.pos)
	  	this.vel = [Math.sign(nextMove[0] - this.pos[0]) * 2, Math.sign(nextMove[1] - this.pos[1]) * 2]; 
	  	this.pos[0] = this.pos[0] + this.vel[0];
  		this.pos[1] = this.pos[1] + this.vel[1];
...
var Inky = App.MovingObjects.Inky = function Inky(pos, vel, sprites, startingPos, options){
  ...
  Inky.prototype.move = function() {
  	...
  	if(!pinky.moved || pinky.animationCycle < 16) {
  		return;
  	}
...
var Clyde = App.MovingObjects.Clyde = function Clyde(pos, vel, sprites, startingPos, options){
  ...
  Clyde.prototype.move = function() {
  	...
  	if(!inky.moved || inky.animationCycle < 16) {
  		return;
  	}
