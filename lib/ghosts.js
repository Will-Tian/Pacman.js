(function() {
  ...
  var Ghost = App.MovingObjects.Ghost = function Ghost(options) {
  	MovingObjects.call(this, options.pos, options.vel, options.sprites, options.ptx, options.gtx, options.blinkytx, options.pinkytx, options.inkytx, options.clydetx);
  	this.path = [];
  }
  ...
  Blinky.prototype.draw = function(){
   ...
   //  var pacman = App.MovingObjects.Objects['pacman'];	
    ...
    this.move();
  	...
  	// 	var pathX = node[0];
  	// 	var pathY = node[1];
  	// 	this.blinkytx.fillStyle = 'red';
  	// 	this.blinkytx.fillRect(pathX - App.Blocks.BLOCK_DIM[0]/2, pathY - App.Blocks.BLOCK_DIM[1]/2, App.Blocks.BLOCK_DIM[0], App.Blocks.BLOCK_DIM[1]);
  	// }.bind(this))	
  }
