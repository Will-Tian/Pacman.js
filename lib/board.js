(function() {
  ...
  var Board = Boards.Board = function(grid, ctx, gtx, ptx, sprites){
    this.grid = grid;
    this.ctx = ctx;
    this.gtx = gtx;
    this.ptx = ptx;
  ...
  Board.prototype.draw = function() {
    for(var y=0;y<this.grid.length;y++) {
      for(var x=0;x<this.grid[0].length;x++){
        ...
        var posX = $('canvas').position().left + 1 + x * BLOCK_DIM[0];
        var posY = $('canvas').position().top + 1 + y * BLOCK_DIM[1];
        if(this.grid[y][x] === "P") {
          var pacman = new App.MovingObjects.Pacman({pos: [posX, posY], vel: [0, 0], sprites: this.sprites, ptx: this.ptx});
